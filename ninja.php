...
elseif ($mode=="-p" || $mode=="-ps") {
  ...
  elseif($format == "NINJA") {
    ...
    if($format == "T" . chr(0xa)) {
      ...
      rup_text_patch($patch, $target, 0. $silent);
    }
    elseif($format == "TZ") {
      print "Patch identified as compressed, textual RUP...\n";
      rup_text_patch($patch, $target, 1, $silent);
    }
    elseif($format == "B ") {
      print "Patch identified as binary RUP...\n";
      rup_bin_patch($patch, $target, 0, $silent);
    }
    elseif($format == "BZ") {
      print "Patch identified as compressed, binary RUP...\n";
      rup_bin_patch($patch, $target, 1, $silent);
    }
  }
  else
...
function rup_text_patch($infile, $outfile, $compressed, $silent) {
...
function ips_patch($infile, $outfile, $silent) {
...
function DisplayOptions() {
  ...
  Apply a patch (use -ps to automatically delete the backup file without prompting the user):
